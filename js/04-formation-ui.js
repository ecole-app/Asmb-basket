...
function openSeanceEditModal(cyId,seaNum){
  ...
  inner.innerHTML=
    ...
    (existing&&!existingContent&&existing.sits&&existing.sits.length
      ? '<div style="font-size:11px;color:var(--mut);background:#e6f0e9;border-radius:var(--rx);padding:10px 12px;margin-bottom:12px;line-height:1.4">Cette séance contient '+existing.sits.length+' situation(s) détaillée(s) du contenu d\'origine. Elles sont conservées telles quelles.</div>'
